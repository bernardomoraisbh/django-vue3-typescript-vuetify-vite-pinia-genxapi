...
def vite_manifest(entries_names):
	app_name = 'genxapi'
	manifest_filepath = path.join(app_name, 'static/vue/manifest.json')
		...
		def _process_entires(names):
			...
			for name in names:
				if name in _processed:
					continue
				chunk = manifest[name]
				import_scripts, import_styles = _process_entires(chunk.get('imports',[]))
				...
				_processed.add(name)
			return scripts, styles
		return _process_entires(entries_names)
...
def vite_styles(*entries_names):
	"""
		...
		Usage::
			{% vite_styles 'main.js' 'other_entry.js' %}
	"""
